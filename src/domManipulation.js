...
const filterArray= function(arr,str){
    if(str!== "today" && str!=="this week"){
         let filtered= arr.filter((obj)=>(obj.project===str));
         return filtered; 
    }
    if(str==="today"){
        let filtered= arr.filter((obj)=>(obj.date===formatISO(new Date,{ representation: 'date' })));
        return filtered; 
    }
    if(str==="this week"){
        let filtered= arr.filter((obj)=>(obj.date >= formatISO(new Date,{ representation: 'date' }) 
        && obj.date < formatISO(add(new Date,{days:7}),{ representation: 'date' })));
        return filtered; 
    }
}
export {htmlCreate};
